...
public class RoleDaoImpl extends BaseDaoImpl<RoleDomain> implements IRoleDao {
	...
	public List<RoleDomain> find(String textToFind) {
		...
		Criteria criteria = session.createCriteria(RoleDomain.class);
		...
		List<RoleDomain> roles = criteria.list();
		return roles;
	}

	public List<RoleDomain> find2(String textToFind) {
		Integer id = null;
		if (StringUtils.isNumeric(textToFind)) {
			id = Integer.valueOf(textToFind);
		}
		Query q = sessionFactory.getCurrentSession().createQuery("from RoleDomain where _name like :parameter or _id=:id");
